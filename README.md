...
### Video Demo: <URL https://www.youtube.com/bSlope8348> TODO
...
3.	Build a 2d platformer implementing various rules and including UI elements
4.	Integrated a SQLite3 database for logging player actions and scores, and to create a leaderboard. 

-	More than one level was originally in the scope, but was cut for purposes of time. This could be implemented in the future to allow a proper tutorial expience for the player, a proper explanation of the controls, or for multiple levels and leaderboards. 
...
## The Game
The game starts with asking for the current player's name and then allows the player to figure out how to play on their own. Upon trial and error the player will find two playable characters, a coin, a movable box, and a grayed out exit. There are also walls and floors to the level. Then latter of which has multiple options (one of which the player can fall through). 

Ultimately, the player will find out they will need to collect the coin with one of the characters and that character will need to reach the exit to complete the level. There is also a timer in the level to let the player know how long it has take them. 

A pause menu is also implemented with multiple options: resume, save, load, top scores, restart, and exit. While paused game timer will also pause. The following is an explanation of each item:
...
4.	Top Scores: Shows the player the top 10 shortest times on their machine (local SQL database).
...
INSERT INTO log (name, time_completed, first_key_used, date_logged) VALUES ('%s', %f, '%s', %d), escapeSQLString(playerName), timeCompleted, escapeSQLString(firstKey), os.time();

SELECT COUNT(*) as rank FROM log WHERE time_completed > 0 AND time_completed < %f AND time_completed IS NOT NULL, timeCompleted;

SELECT * FROM log WHERE time_completed > 0 AND time_completed IS NOT NULL ORDER BY time_completed ASC LIMIT 10;
...
-	**pause.lua**: The pause screen that handles all the menu actions. This included linking to save/load and pulling the database to show top scorces.
-	**scoreBox.lua**: Used to create a timing score for the user to see. 

#### Main Libraries:
-	classic.lua: Used to create classed like entity.lua.
...
### Building the Game:
**makelove** is a packaging too for love games that was used to build the source code into executables for different platforms. 
